...
class Mortgage(object):
    ...
    def __init__(self, loanID, lenderName, loan_status, loan_sub_status, borrower, \
                     period, remaining_months, unpaidPrincipal, balance, annualRate, prepaymentArr) -> None:
    ...
    def refinance(self, prepaymentArray, newRate, month):
        ...

    def speed(self):
        months_in = self.period - self.remaining_months # returns the month that the person is at
        scheduledBalance = self.owed[months_in - 1] # returns the balance that is owed at that month in time
        speed = (scheduledBalance - self.unpaidPrincipal) / scheduledBalance
        if scheduledBalance > self.unpaidPrincipal:
            status = "AHEAD"
        elif scheduledBalance < self.unpaidPrincipal:
            status = "BEHIND"
        else:
            status = "ON TIME"
        return (status, round(100 * speed, 6))
...
def main():
    mortgage1 = Mortgage(1221130638, "Shore", "Current", "Current", "Paige", 360, 350, 99000, 100000, 0.03, [11])
    print(mortgage1)
    print(mortgage1.speed()[0])
